...
class Scraper:
    ...
    async def _cleanup_page(self, page: Optional[Page]) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to close page: {type(e).__name__}: {str(e)}")
            self.logger.debug(f"Close page exception details: {repr(e)}")
    ...
    def __init__(self, user_data_dir: str | None = None) -> None:
        ...
        self._tab_semaphore: asyncio.Semaphore = asyncio.Semaphore(10)
    ...
    async def scrape_async(
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                # Rate limit first, then acquire tab semaphore and create new page
                ...
                # Clean up the page
                if page:
                    asyncio.create_task(self._cleanup_page(page))
